...
Upload benchmarks
=================
The following results are a study about the performance of tg-up uploading files. The results are not
...
size was the default in tg-up. The chunk size vary depending on the file size:
...
tg-up uploads *4 chunks at the same time*. You can change this value using the ``PARALLEL_UPLOAD_BLOCKS``
