...
from tg_up.utils import grouper, async_to_sync, get_environment_integer

PARALLEL_UPLOAD_BLOCKS = get_environment_integer('TG_UP_PARALLEL_UPLOAD_BLOCKS', 4)
ALBUM_FILES = 10
RETRIES = 3
MAX_RECONNECT_RETRIES = get_environment_integer('TG_UP_MAX_RECONNECT_RETRIES', 5)
RECONNECT_TIMEOUT = get_environment_integer('TG_UP_RECONNECT_TIMEOUT', 5)
MIN_RECONNECT_WAIT = get_environment_integer('TG_UP_MIN_RECONNECT_WAIT', 2)
        ...
        async with helpers._FileStream(file, file_size=file_size) as stream:
            ...
            for part_index in range(part_count):
                ...
                self.loop.create_task(
                    ...
                    name=f"tg-up-file-{part_index}"
                )
            # Wait for all tasks to finish
            await asyncio.wait([
                task for task in asyncio.all_tasks() if task.get_name().startswith(f"tg-up-file-")
            ])
        if is_big:
